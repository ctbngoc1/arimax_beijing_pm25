...
>
> As a compromise between these two approaches, **m-step forecasting** (m\>1) uses all real observations up to time T to forecast the observations at time points T+1, …, T+m. Once the actual observations for these m time periods become available, the model is updated and used to forecast the next m observations. Therefore, m-step forecasting generally produces more accurate forecasts than multi-step forecasting while requiring less computational effort than one-step forecasting.
>
> During one-step (or m-step) forecasting for ARIMAX, it’s possible to refit the model at every step (or every m steps) or just update the internal state (Kalman filter) instead to reduce runtime.

Model performance was first evaluated using multi-step forecasting, which yielded a test RMSE of 121.32 and a test MAE of 102.65. However, because forecasting errors tend to accumulate over longer forecasting horizons, additional evaluation using a 24-step forecasting strategy, in which the model's internal state was updated every 24 steps, was conducted to assess whether more frequent model updates can improve predictive performance.

For the m-step forecasting strategy, m = 24 was selected since updating the model every few hours (by choosing m = 1, 6, or 12) is unrealistic in practice. When the time between model updates is too short, the forecasting task becomes less meaningful and more closely resembles a tracking task.
...
Overall, the results indicated that the model was less effective than the naive forecasting benchmark at short forecasting horizons but became increasingly competitive as the forecasting horizon increased up to h = 24. This is not surprising, as naive forecasting models are typically very strong at short forecasting horizons.
